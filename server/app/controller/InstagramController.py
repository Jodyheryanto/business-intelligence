...
def printData(tanggal_awal, tanggal_akhir, sentiments, user_id):
    try:
        posts = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==3).all()
        Word_Clouds.query.filter(Word_Clouds.user_id==user_id).filter(Word_Clouds.source_id==3).delete(synchronize_session=False)
        db.session.commit()
...
def printAllData(tanggal_awal, tanggal_akhir, sentiments):
    try:
        posts = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.source_id==3).all()
        data = transform(posts, sentiments)
        temps = []
        for i in range(len(data)):
            for sentiment in sentiments:
                if(data[i]['sentiment'] == int(sentiment)):
                    temps.append(data[i]['post'])
        Word_Clouds.query.filter(Word_Clouds.user_id==1).filter(Word_Clouds.source_id==3).delete(synchronize_session=False)
        db.session.commit()
        if(len(temps) > 0):
            ngrams = count_ngrams(temps)
...
def index(tanggal_awal, tanggal_akhir):
    try:
        api = API_Config.query.filter_by(id=1).first()
        data = []
        sentiment = []
        text = []
        temps = []
        ...
        for user in users:
            print(user.keyword_ig)
            if(user.keyword_ig != ''):
                hasil = requests.get(api.url_api + user.keyword_ig + api.param1)
                hasil = hasil.json()
                temps = []
                if(int(hasil['graphql']['hashtag']['edge_hashtag_to_media']['count']) > 0):
                    for j in range(int(hasil['graphql']['hashtag']['edge_hashtag_to_media']['count'])):
                        hasil_detail = requests.get("https://www.instagram.com/p/"+ hasil['graphql']['hashtag']['edge_hashtag_to_media']['edges'][j]['node']['shortcode'] +"/?__a=1")
                        if(hasil_detail.status_code == 200):
                            hasil_detail = hasil_detail.json()
                            d = datetime.fromtimestamp(hasil_detail['graphql']['shortcode_media']['taken_at_timestamp'])
                            tanggal_post = d.strftime('%Y-%m-%d')
                            if(tanggal_post > tanggal_awal and tanggal_post < tanggal_akhir):
                                user_name = hasil_detail['graphql']['shortcode_media']['owner']['username'].encode('ascii', 'ignore')
                                user_avatar = hasil_detail['graphql']['shortcode_media']['owner']['profile_pic_url']
                                post = hasil_detail['graphql']['shortcode_media']['edge_media_to_caption']['edges'][0]['node']['text'][:500]
                                post = post.encode('ascii', 'ignore')
                                sumber = 3
                                link_review = "https://www.instagram.com/p/"+ hasil['graphql']['hashtag']['edge_hashtag_to_media']['edges'][j]['node']['shortcode']
                                rating = "-"
                                # sentiment = model_svm(hasil['feed']['entry'][j]['content']['label'])
                                data.append({ "user_name": user_name, "user_avatar": user_avatar, "post": post, "tanggal_post": tanggal_post, "sumber": sumber, "user_id": user.id, "link_review": link_review, "rating": rating })
                                text.append(hasil_detail['graphql']['shortcode_media']['edge_media_to_caption']['edges'][0]['node']['text'])
                                temps.append(hasil_detail['graphql']['shortcode_media']['edge_media_to_caption']['edges'][0]['node']['text'])
                        ...
                        if(len(data) > 0):
                            ...
                            for i in range(len(data)):
                                ...
                                if(len(data[i]['post']) > 500):
                                    post = data[i]['post'][:500] + "..."
                                else:
                                    post = data[i]['post']
                                ...
                                posts = Posts(username=user_name, user_avatar=user_avatar, post=post, tanggal_post=tanggal_post, source_id=sumber,
                                user_id=user_id, link_review=link_review, rating=rating, sentiment=sentiment)
                                db.session.add(posts)
                                db.session.commit()
        post_var = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.source_id==3).all()
        if len(post_var) > 0:
            return post_var
        else:
...
def print_most_frequent(tanggal_awal, tanggal_akhir, ngrams, user_id, num=10):
    for n in sorted(ngrams):
        for gram, count in ngrams[n].most_common(num):
            ...
            post_sentiment = Posts.query.with_entities(func.avg(Posts.sentiment).label('avg_sentiment')).filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==3).filter(Posts.post.like(search)).all()
            post_id = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==3).filter(Posts.post.like(search)).first()
            sentiment = int(post_sentiment[0].avg_sentiment)
            sample_id = post_id.id
            word_clouds = Word_Clouds(word=word, user_id=user_id, source_id=3, value=value, sentiment=sentiment, sample_id=sample_id)
...
def print_all_most_frequent(tanggal_awal, tanggal_akhir, ngrams, num=10):
    for n in sorted(ngrams):
        for gram, count in ngrams[n].most_common(num):
            ...
            # Lanjutkan perihal select max value
            # post_sample = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.aplikasi==aplikasi).filter(Posts.post.like(search)).all()
            post_sentiment = Posts.query.with_entities(func.avg(Posts.sentiment).label('avg_sentiment')).filter(Posts.source_id==3).filter(Posts.post.like(search)).all()
            post_id = Posts.query.filter(Posts.post.like(search)).filter(Posts.source_id==3).first()
            sentiment = int(post_sentiment[0].avg_sentiment)
            sample_id = post_id.id
            word_clouds = Word_Clouds(word=word, user_id=1, source_id=3, value=value, sentiment=sentiment, sample_id=sample_id)
            db.session.add(word_clouds)
            db.session.commit()
