...
def printData(tanggal_awal, tanggal_akhir, sentiments, user_id):
    try:
        posts = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==1).all()
        Word_Clouds.query.filter(Word_Clouds.user_id==user_id).filter(Word_Clouds.source_id==1).delete(synchronize_session=False)
        db.session.commit()
...
def printAllData(tanggal_awal, tanggal_akhir, sentiments):
    try:
        posts = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.source_id==1).all()
        data = transform(posts, sentiments)
        temps = []
        for i in range(len(data)):
            for sentiment in sentiments:
                if(data[i]['sentiment'] == int(sentiment)):
                    temps.append(data[i]['post'])
        Word_Clouds.query.filter(Word_Clouds.user_id==1).filter(Word_Clouds.source_id==1).delete(synchronize_session=False)
        db.session.commit()
        # Word_Clouds.query.filter(Word_Clouds.aplikasi=='all').filter(Word_Clouds.sumber=="Google Play").delete()
        db.session.commit()
        if(len(temps) > 0):
            ngrams = count_ngrams(temps)
...
def index(tanggal_awal, tanggal_akhir):
    try:
        api = API_Config.query.filter_by(id=3).first()
        users = Users.query.filter_by(is_active=1).all()
        for user in users:
            print(user.app_google_play)
            if(user.app_google_play != ''):
                data = []
                sentiment = []
                text = []
                temps = []
                # for i in aplikasi:
                hasil = requests.get(api.url_api + user.app_google_play + api.param1)
                hasil = hasil.json()
                temps = []
                if(len(hasil['results']) > 0):
                    for j in range(len(hasil['results'])):
                        d = datetime.fromisoformat(hasil['results'][j]['date'][:-1])
                        tanggal_post = d.strftime('%Y-%m-%d')
                        if(tanggal_post > tanggal_awal and tanggal_post < tanggal_akhir):
                            user_name = hasil['results'][j]['userName'].encode('ascii', 'ignore')
                            user_avatar = hasil['results'][j]['userImage']
                            post = hasil['results'][j]['text'].encode('ascii', 'ignore')
                            sumber = 1
                            link_review = hasil['results'][j]['url']
                            rating = hasil['results'][j]['scoreText']
                            # sentiment = model_svm(hasil['results'][j]['text'])
                            data.append({ "user_name": user_name, "user_avatar": user_avatar, "post": post, "tanggal_post": tanggal_post,
                            "sumber": sumber, "user_id": user.id, "link_review": link_review, "rating": rating })
                    ...
                    if(len(data) > 0):
                        ...
                        for i in range(len(data)):
                            ...
                            source_id = data[i]['sumber']
                            user_id = data[i]['user_id']
                            link_review = data[i]['link_review']
                            rating = data[i]['rating']
                            sentiment = sentiments[i]
                            posts = Posts(username=user_name, user_avatar=user_avatar, post=post, tanggal_post=tanggal_post, source_id=source_id,
                            user_id=user_id, link_review=link_review, rating=rating, sentiment=sentiment)
                            db.session.add(posts)
                            db.session.commit()
        post_var = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.source_id==1).all()
        if len(post_var) > 0:
            return post_var
        else:
...
def print_most_frequent(tanggal_awal, tanggal_akhir, ngrams, user_id, num=10):
    for n in sorted(ngrams):
        for gram, count in ngrams[n].most_common(num):
            ...
            post_sentiment = Posts.query.with_entities(func.avg(Posts.sentiment).label('avg_sentiment')).filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==1).filter(Posts.post.like(search)).all()
            post_id = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==1).filter(Posts.post.like(search)).first()
            sentiment = int(post_sentiment[0].avg_sentiment)
            sample_id = post_id.id
            word_clouds = Word_Clouds(word=word, user_id=user_id, source_id=1, value=value, sentiment=sentiment, sample_id=sample_id)
...
def print_all_most_frequent(tanggal_awal, tanggal_akhir, ngrams, num=10):
    for n in sorted(ngrams):
        for gram, count in ngrams[n].most_common(num):
            ...
            # Lanjutkan perihal select max value
            # post_sample = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.aplikasi==aplikasi).filter(Posts.post.like(search)).all()
            post_sentiment = Posts.query.with_entities(func.avg(Posts.sentiment).label('avg_sentiment')).filter(Posts.source_id==1).filter(Posts.post.like(search)).all()
            post_id = Posts.query.filter(Posts.post.like(search)).filter(Posts.source_id==1).first()
            sentiment = int(post_sentiment[0].avg_sentiment)
            sample_id = post_id.id
            word_clouds = Word_Clouds(word=word, user_id=1, source_id=1, value=value, sentiment=sentiment, sample_id=sample_id)
            db.session.add(word_clouds)
            db.session.commit()