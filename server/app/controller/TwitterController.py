...
def printData(tanggal_awal, tanggal_akhir, sentiments, user_id):
    try:
        posts = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==4).all()
        Word_Clouds.query.filter(Word_Clouds.user_id==user_id).filter(Word_Clouds.source_id==4).delete(synchronize_session=False)
        db.session.commit()
        ...
        # Word_Clouds.query.filter(Word_Clouds.aplikasi==nama_aplikasi).filter(Word_Clouds.aplikasi=="Twitter").delete()
        db.session.commit()
        if(len(temps) > 0):
        ...
        print(e)
        return response.badRequest([], 'Data failed to be processed')

@jwt_required
def printAllData(tanggal_awal, tanggal_akhir, sentiments):
    try:
        posts = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.source_id==4).all()
        data = transform(posts, sentiments)
        temps = []
        for i in range(len(data)):
            for sentiment in sentiments:
                if(data[i]['sentiment'] == int(sentiment)):
                    temps.append(data[i]['post'])
        Word_Clouds.query.filter(Word_Clouds.user_id==1).filter(Word_Clouds.source_id==4).delete(synchronize_session=False)
        db.session.commit()
        # Word_Clouds.query.filter(Word_Clouds.aplikasi=='all').filter(Word_Clouds.sumber=="Twitter").delete()
        db.session.commit()
        if(len(temps) > 0):
            ngrams = count_ngrams(temps)
...
def index(tanggal_awal, tanggal_akhir):
    try:
        api = API_Config.query.filter_by(id=2).first()
        data = []
        sentiment = []
        text = []
        temps = []
        ...
        for user in users:
            print(user.keyword_twitter)
            if(user.keyword_twitter != ''):
                response = oauth.get(api.url_api + user.keyword_twitter , params = params)
                hasil = response.json()
                temps = []
                if(len(hasil['statuses']) > 0):
                    for j in range(len(hasil['statuses'])):
                        d = datetime.strptime(hasil['statuses'][j]['created_at'], '%a %b %d %H:%M:%S +0000 %Y')
                        tanggal_post = d.strftime('%Y-%m-%d')
                        if(tanggal_post > tanggal_awal and tanggal_post < tanggal_akhir):
                            user_name = hasil['statuses'][j]['user']['name'].encode('ascii', 'ignore')
                            user_avatar = hasil['statuses'][j]['user']['profile_image_url']
                            post = hasil['statuses'][j]['text']
                            post = post.encode('ascii', 'ignore')
                            sumber = 4
                            if(len(hasil['statuses'][j]['entities']['urls']) != 0):
                                link_review = hasil['statuses'][j]['entities']['urls'][0]['url']
                            else:
                                link_review = "-"
                            rating = "-"
                            ...
                            text.append(hasil['statuses'][j]['text'])
                            temps.append(hasil['statuses'][j]['text'])
                    ...
                    if(len(data) > 0):
                        ...
                        for i in range(len(data)):
                            ...
                            db.session.add(posts)
                            db.session.commit()
        post_var = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.source_id==4).all()
        if len(post_var) > 0:
            return post_var
        else:
...
def print_most_frequent(tanggal_awal, tanggal_akhir, ngrams, user_id, num=10):
    for n in sorted(ngrams):
        for gram, count in ngrams[n].most_common(num):
            ...
            post_sentiment = Posts.query.with_entities(func.avg(Posts.sentiment).label('avg_sentiment')).filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==4).filter(Posts.post.like(search)).all()
            post_id = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.user_id==user_id).filter(Posts.source_id==4).filter(Posts.post.like(search)).first()
            sentiment = int(post_sentiment[0].avg_sentiment)
            sample_id = post_id.id
            word_clouds = Word_Clouds(word=word, user_id=user_id, source_id=4, value=value, sentiment=sentiment, sample_id=sample_id)
...
def print_all_most_frequent(tanggal_awal, tanggal_akhir, ngrams, num=10):
    for n in sorted(ngrams):
        for gram, count in ngrams[n].most_common(num):
            ...
            # Lanjutkan perihal select max value
            # post_sample = Posts.query.filter(Posts.tanggal_post.between(tanggal_awal, tanggal_akhir)).filter(Posts.aplikasi==aplikasi).filter(Posts.post.like(search)).all()
            post_sentiment = Posts.query.with_entities(func.avg(Posts.sentiment).label('avg_sentiment')).filter(Posts.source_id==4).filter(Posts.post.like(search)).all()
            post_id = Posts.query.filter(Posts.post.like(search)).filter(Posts.source_id==4).first()
            sentiment = int(post_sentiment[0].avg_sentiment)
            sample_id = post_id.id
            word_clouds = Word_Clouds(word=word, user_id=1, source_id=4, value=value, sentiment=sentiment, sample_id=sample_id)
            db.session.add(word_clouds)
            db.session.commit()
