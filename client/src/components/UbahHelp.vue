<template>
  ...
  <div class="example-modal-content">
      <h3 style="margin: 15px 20px;">Form Ubah Help</h3>
      <hr>
...
export default {
  ...
  mounted() {
    this.$eventBus.$on("datafromtableHelp", (x) => {
      this.neww = x;
      this.form.title = this.neww.title
      this.form.description = this.neww.description
      store.state.help.param.id = this.neww.id
    });
  },
  methods:{
      ...mapActions({
        changeNotif: 'help/changeHelp'
      }),
      async submitForm(){
          this.isLoading = true
