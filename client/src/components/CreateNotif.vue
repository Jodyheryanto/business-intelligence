<template>
  ...
  <div class="example-modal-content">
      <h3 style="margin: 15px 20px;">Form Tambah Notification</h3>
      <hr>
...
export default {
  ...
  methods:{
      ...mapActions({
        createNotif: 'notification/createNotif'
      }),
      async submitForm(){
          this.isLoading = true
          if(this.form.title && this.form.description){
            store.state.notification.param_notif = this.form
            store.state.notification.param_notif.access_token = this.$cookie.get('user_token')
            this.createNotif().then(() => {
