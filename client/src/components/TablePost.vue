    ...
    <div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <h4 style="color: red; margin-top: 20px;" class="text-center" v-else>Data tidak ada</h4>
            </div>
            </div>
        </div>
        </div>
...
export default {
    ...
    mounted() {
        this.sources = []
        this.source = this.source = this.$cookie.get('source0');
        ...
        fillData(){
            ...
            for(var i = 0; i < data.length; i++){
                ...
                //this.items.push({isActive: true, date: data.book.critic_reviews[i].review_date, source: data.book.critic_reviews[i].source, rating: data.book.critic_reviews[i].star_rating, sentiment: data.book.critic_reviews[i].pos_or_neg, post: data.book.critic_reviews[i].snippet, link_review: data.book.critic_reviews[i].review_link,account: { username: this.users[i].name, image: this.users[i].avatar }})
                    this.items.push({id: data[i].id, account: { username: data[i].username, user_avatar: data[i].user_avatar }, post: data[i].post, tanggal_post: moment(data[i].tanggal_post).format('DD MMM YYYY'), sumber: data[i].sumber, link_review: data[i].link_review, rating: data[i].rating, sentiment: data[i].sentiment})
                }
            }
